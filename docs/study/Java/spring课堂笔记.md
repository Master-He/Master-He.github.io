...
![1078856-20170205160357354-490660449](	.assets/3.png)
...
Spring的声明式事务顾名思义就是采用声明的方式来处理事务。这里所说的声明，就是指在配置文件中声明。用在Spring配置文件中声明式的处理事务来代替代码式的处理事务。这样的好处是，事务管理不侵入开发的组件

Spring使用[AOP](https://baike.baidu.com/item/AOP)来完成声明式的事务管理
